...
1. Replace Column C (Grade) with self-submitted grade
2. Move weighted grade (rounded up) to column I
3. Replace column H with detailed feedback report text (with opening message)
"""
import openpyxl
import json
import math
...
def generate_feedback_text(assessment_data):
    ...
    if total_score >= 80:
        ...
        tone = "good potential with room for growth"
    else:
        tone = "developing skills that show promise"
    ...
    if weaknesses and weaknesses[0] != "No critical weaknesses (<5/10)":
        ...
        if len(weakness_names) > 0:
            feedback += f"Focus on improving: {', '.join(weakness_names)}. "
    ...
    if total_score >= 80:
        feedback += "Continue building on your strong foundation for future assignments."
    elif total_score >= 55:
        feedback += "With focused improvements in these areas, you can excel in future assignments."
    else:
        feedback += "Focus on the priority areas outlined - you have strong potential for growth."
...
def update_assignment1_grades():
    ...
    for row_idx in range(2, ws.max_row + 1):
        # Column A has format "משתתף:38950"
        identifier = ws[f'A{row_idx}'].value

        if not identifier:
            continue

        # Extract student ID from "משתתף:XXXXX" format
        student_id = None
        ...
        if not student_id:
            print(f"Row {row_idx}: Could not extract student ID from {identifier}")
            continue
        ...
        if assessment:
            ...
            weighted_grade_rounded = math.ceil(weighted_grade)

            # Column C: Replace with self-grade (or weighted if no self-grade)
            if self_grade is not None:
                ws[f'C{row_idx}'] = self_grade
                updates['self_grades_found'] += 1
                grade_used = self_grade
            else:
                ws[f'C{row_idx}'] = weighted_grade_rounded
                updates['self_grades_missing'] += 1
                grade_used = weighted_grade_rounded

            # Column I: Weighted grade (rounded up) - "Grade you would have gotten"
            ws[f'I{row_idx}'] = weighted_grade_rounded
            ...
            print(f"Row {row_idx}: Student {student_id} - Used grade: {grade_used}, Calculated: {weighted_grade_rounded}")
        else:
            print(f"Row {row_idx}: Student {student_id} - No assessment data found")

    # Update column header for I
    ws['I1'] = 'Calculated Grade (Tier 2)'

    # Save workbook to new file (file might be locked)
    ...
    print(f"Self-grades found and used: {updates['self_grades_found']}")
    print(f"Self-grades missing (used calculated): {updates['self_grades_missing']}")
    print(f"Feedback messages added: {updates['feedback_added']}")
    print(f"\nFile updated: {output_path}")
    print("=" * 80)
